...
class Product(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    name: str
    description: Optional[str] = None   # ← коштук
    price: float
...
async def create_order(order: OrderCreate):
    ...
    order_number = str(today_count + 1).zfill(3)  # 001, 002, ...

    order_obj = Order(**order.dict(), order_number=order_number)  # <-- order_number кошулду
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://cafe-delicious.online","https://www.cafe-delicious.online" ],
